...
def affine_backward(dout, cache):
    ...
    d_input = dout.dot(w.T)
    dx = d_input.reshape(x.shape)
    dw = input.T.dot(dout)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    F, C, HH, WW = w.shape
    H_filtered = 1 + (H - HH) // stride
    W_filtered = 1 + (W - WW) // stride   
    
    out = np.zeros((N, F, H_filtered, W_filtered))
    
    for n in range(N):
        for f in range(F):
            for h in range(0, H - HH + 1, stride):
                for width in range(0, W - WW + 1, stride):
                    out[n, f, int(h/stride), int(width/stride)] += np.sum(x_padded[n, :, h:h+HH, width:width+WW] * w[f]) + b[f]
...
def conv_backward_naive(dout, cache):
    ...
    for n in range(N):
        for f in range(F):
            for h in range(0, H - HH + 1, stride):
                for width in range(0, W - WW + 1, stride):
                    dw[f, :, :, :] += x_padded[n, :, h:h+HH, width:width+WW] * dout[n, f, int(h/stride), int(width/stride)]
                    dx_padded[n, :, h:h+HH, width:width+WW] += w[f] * dout[n, f, int(h/stride), int(width/stride)]
                    
    
    db = [np.sum(dout[:, f, :, :]) for f in range(F)]
...
def max_pool_forward_naive(x, pool_param):
    ...
    H_filtered = 1 + (H - pool_height) // stride
    W_filtered = 1 + (W - pool_width) // stride
    
    out = np.zeros((N, C, H_filtered, W_filtered))
    
    for n in range(N):
        for c in range(C):
            for h in range(0, H - pool_height + 1, stride):
                for width in range(0, W - pool_width + 1, stride):
                    out[n, c, int(h/stride), int(width/stride)] += np.max(x[n, c, h:h+pool_height, width:width+pool_width])
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for h in range(0, H - pool_height + 1, stride):
                for width in range(0, W - pool_width + 1, stride):
                    ind = np.unravel_index(np.argmax(x[n, c, h:h+pool_height, width:width+pool_width], axis=None), (pool_height, pool_width))
                    dx[n, c, h:h+pool_height, width:width+pool_width][ind] += dout[n, c, int(h/stride), int(width/stride)]
...
def avg_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
        for c in range(C):
            for h in range(0, H - pool_height + 1, stride):
                for width in range(0, W - pool_width + 1, stride):
                    out[n, c, int(h/stride), int(width/stride)] += np.mean(x[n, c, h:h+pool_height, width:width+pool_width])
...
def avg_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for h in range(0, H - pool_height + 1, stride):
                for width in range(0, W - pool_width + 1, stride):
                    dx[n, c, h:h+pool_height, width:width+pool_width] += 1/(pool_height * pool_width) * dout[n, c, int(h/stride), int(width/stride)]
