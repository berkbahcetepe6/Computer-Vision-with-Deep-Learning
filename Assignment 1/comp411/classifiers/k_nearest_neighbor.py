...
class KNearestNeighbor(object):
    ...
    def compute_L2_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                powered = np.power(self.X_train[j] - X[i], 2)
                dists[i][j] = np.sqrt(np.sum(powered))
    ...
    def compute_L2_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            powered = np.power(self.X_train - X[i], 2)
            dists[i, :] = np.sqrt(np.sum(powered, axis=1))
    ...
    def compute_L2_distances_no_loops(self, X):
        ...
        ##From the discussion session we can use the formula:
        # (x - y)^2 = x^2 - 2xy + y^2

        X_train_squared = np.sum(np.power(self.X_train, 2), axis=-1)[:,np.newaxis]
        X_test_squared = np.sum(np.power(X, 2), axis=-1)[:,np.newaxis]
        X_tr_test = X @ self.X_train.T

        dists = np.sqrt(X_test_squared + X_train_squared.T - 2*X_tr_test)
    ...
    def compute_Cosine_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                #####################################################################
                # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
                nomin = np.sum(self.X_train[j] * X[i])
                denom = np.sqrt(np.sum(np.power(self.X_train[j], 2))) * np.sqrt(np.sum(np.power(X[i], 2)))

                dists[i][j] = 1 - (nomin/denom)
    ...
    def compute_Cosine_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            #######################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            nomin = np.sum(self.X_train * X[i], axis=1)
            denom = np.sqrt(np.sum(np.power(self.X_train, 2), axis=1)) * np.sqrt(np.sum(np.power(X[i], 2)))
            
            dists[i, :] = 1 - (nomin/denom)
    ...
    def compute_Cosine_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        nom = X.dot(self.X_train.T)
        denom = np.sqrt(np.sum(np.power(X,2), axis=1)[:,np.newaxis].dot(np.sum(np.power(self.X_train, 2), axis=1)[:,np.newaxis].T))
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            closest_ones = np.argsort(dists[i])[:k]
            closest_y = self.y_train[closest_ones]
